...
func (e *executor) Run() (err error) {
	l, err := net.Listen("tcp", fmt.Sprintf(":%d", *e.opts.ExecutorPort))
	if err != nil {
		glog.Error(err)
	}

	if *e.opts.ExecutorPort == 0 {
		*e.opts.ExecutorPort = l.Addr().(*net.TCPAddr).Port
	}
	...
	// 监听端口并提供服务
	e.log.Info(fmt.Sprintf("xxljob run：%d", *e.opts.ExecutorPort))
	go server.Serve(l)
	go func() {
...
func (e *executor) runTask(writer http.ResponseWriter, request *http.Request) {
	...
	if e.runList.Exists(Int64ToStr(param.JobID)) {
		...
		if oldTask != nil {
			// 任务运行时间（秒）
			runSeconds := oldTask.TimeConsuming()

			// 如果阻塞处理策略是覆盖之前调度，或者任务已经超时，删除任务队列重新调度
			// 注：删除任务队列，不代表上一次调度任务取消，可能任务也在执行中
			if param.ExecutorBlockStrategy == coverEarly || (param.ExecutorTimeout > 0 && runSeconds > param.ExecutorTimeout) {
				e.runList.Del(Int64ToStr(oldTask.Id))
				if oldTask.Cf != nil {
					oldTask.Cf()
				}
				msg := fmt.Sprintf("[%d，%s]任务已运行%ds，已超过设置的超时时间%ds，将重新调度。", oldTask.Id, oldTask.Name, runSeconds, param.ExecutorTimeout)
				glog.Error(msg)
				e.callback(oldTask, 500, msg)
			} else {
				//单机串行,丢弃后续调度 都进行阻塞
				msg := fmt.Sprintf("当前任务正在运行中，任务开始时间为：%s，已运行：%ds", oldTask.FormatStartTime(), runSeconds)
				_, _ = writer.Write(returnCall(param, 400, msg))
				return
			}
		}
	}
	...
	if param.ExecutorTimeout > 0 {
		task.Ctx, task.Cf = context.WithTimeout(cxt, time.Duration(param.ExecutorTimeout)*time.Second)
	} else {
		task.Ctx, task.Cf = context.WithCancel(cxt)
	}
	task.Id = param.JobID
	...
	go task.Run(task.Param.LogID, func(code int64, msg string) {
		e.callback(task, code, msg)
	})
	// e.log.Info("任务["+Int64ToStr(param.JobID)+"]开始执行:"+param.ExecutorHandler, "参数：", string(req))
...
func (e *executor) registry() {
	...
	req := &Registry{
		...
		RegistryValue: fmt.Sprintf("http://%s:%d", e.opts.ExecutorIp, *e.opts.ExecutorPort),
	}
...
func (e *executor) post(action, body string) (resp *http.Response, err error) {
	...
	return utils.HttpDefaultClient.Do(request)
}
